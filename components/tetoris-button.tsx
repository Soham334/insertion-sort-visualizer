...
export default function TetorisButton({ label, onClick, color }: TetorisButtonProps) {
  ...
  return (
    <motion.div className="relative w-full h-12 group" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
      ...
      <div className={`absolute inset-0 translate-y-2 ${scheme.shadow} border-4 border-black z-[-1]`}></div>
      ...
      >
        ...
        <button
          onClick={onClick}
          className={`w-full h-full flex items-center justify-center font-bold text-sm ${scheme.text} tracking-wider`}
